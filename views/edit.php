...
<?php 
	...
	if(!empty(edit_task())){
		...
		echo '<button formaction = "/save_or_del/'.edit_task()['id'].'" name = "save" value = "'.edit_task()['id'].'">Сохранить</button>'; 
		echo '<button formaction = "/save_or_del/'.edit_task()['id'].'" name = "del" value = "'.edit_task()['id'].'">Удалить</button></div></div></form>';
		unset($_SESSION['edit']);
	}
?>